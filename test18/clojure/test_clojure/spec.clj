...
(deftest coll-form
  (are [spec form]
    (= (s/form spec) form)
    (s/map-of int? any?)
    '(clojure.spec.alpha/map-of clojure.future/int? clojure.future/any?)

    (s/coll-of int?)
    '(clojure.spec.alpha/coll-of clojure.future/int?)

    (s/every-kv int? int?)
    '(clojure.spec.alpha/every-kv clojure.future/int? clojure.future/int?)

    (s/every int?)
    '(clojure.spec.alpha/every clojure.future/int?)

    (s/coll-of (s/tuple (s/tuple int?)))
    '(clojure.spec.alpha/coll-of (clojure.spec.alpha/tuple (clojure.spec.alpha/tuple clojure.future/int?)))

    (s/coll-of int? :kind vector?)
    '(clojure.spec.alpha/coll-of clojure.future/int? :kind clojure.core/vector?)

    (s/coll-of int? :gen #(gen/return [1 2]))
    '(clojure.spec.alpha/coll-of clojure.future/int? :gen (fn* [] (gen/return [1 2])))))
